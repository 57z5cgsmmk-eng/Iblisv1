...
local oldIdx, oldNewIdx = nil, nil 
local spoofedVelocity = Vector3.zero 
local billboardCharAddedConn = nil 
local setAntiDropVisual = nil 

-- ============================================================
-- STEAL BAR SCALE VARIABLES
-- ============================================================
local stealBarScale = 1 
local setStealBarScaleVisual = nil 
local stealBarScaleObj = nil 
...
local function setupSpeedIndicator(char) 
    ...
    discordLabel.Text="discord.gg/SrBSCVyKH";discordLabel.TextColor3=Color3.fromRGB(255,255,255) 
...
-- ============================================================
-- ANTI DROP FUNCTIONS (no billboard)
-- ============================================================
local function startAntiDrop() 
    if antiDropActive then return end 
    local ok, mt = pcall(function() return getrawmetatable(game) end) 
    if not ok or not mt then return end 
    if not (setreadonly and newcclosure and checkcaller) then return end 
    antiDropMt = mt 
    oldIdx = mt.__index 
    ...
    mt.__index = newcclosure(function(self, key) 
        if not checkcaller() and (key == "AssemblyLinearVelocity" or key == "Velocity") and 
           ...
           LP.Character and self:IsDescendantOf(LP.Character) then 
...
local function stopAntiDrop() 
    ...
    if antiDropMt and oldIdx then 
        pcall(function() 
            setreadonly(antiDropMt, false) 
            antiDropMt.__index = oldIdx 
            antiDropMt.__newindex = oldNewIdx 
            setreadonly(antiDropMt, true) 
        end) 
        oldIdx = nil 
        oldNewIdx = nil 
    end 
...
function buildMobileButtons() 
    ...
    local g=Instance.new("ScreenGui");g.Name="IblisHub";g.ResetOnSpawn=false;g.DisplayOrder=20;g.IgnoreGuiInset=true 
...
function saveConfig() 
    ...
    local cfg={ 
        ...
        speedToggleKey=ks(KB.SpeedToggle),aimbot2Key=ks(KB.Aimbot2),antiDesyncAimbotKey=ks(KB.AntiDesyncAimbot), 
...
local function buildGui() 
    ...
    local OFF = Color3.fromRGB(28,28,35) 
    local old=game:GetService("CoreGui"):FindFirstChild("IblisHub") or game:GetService("CoreGui"):FindFirstChild(".exe");if old then old:Destroy() end 
    local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild("IblisHub") or pg:FindFirstChild(".exe");if o then o:Destroy() end end 
    local gui=Instance.new("ScreenGui") 
    gui.Name="IblisHub";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true 
    ...
    main.Size=UDim2.new(0,272,0,380);main.Position=UDim2.new(0.5,-136,0.5,-190);exeMainFrame=main 
    ...
    ttl.BackgroundTransparency=1;ttl.Text="IBLIS HUB";ttl.ZIndex=4 
    ...
    for i,t in ipairs(tabs) do 
        ...
        btn.Size=UDim2.new(0,48,0,40);btn.Position=UDim2.new(0,4+(i-1)*50,0.5,-20) 
        ...
        btn.Font=Enum.Font.SciFi;btn.TextSize=8;btn.ZIndex=11 
    ...
    miniBtn.Text="IBLIS";miniBtn.TextColor3=WHITE;miniBtn.Font=Enum.Font.SciFi;miniBtn.TextSize=11 
    ...
    setAntiDropVisual=mkToggle(aimbotPage,"Anti Drop",function(on) 
    ...
    mkSect(configPage,"Steal Bar") 
    do 
        local row=mkRow(configPage,32);mkLabel(row,"Steal Bar Scale") 
        local sizes={50,75,100,125,150} 
        local bw=30 
        ...
        for i,sz in ipairs(sizes) do 
            ...
            sb.BackgroundColor3=(math.floor(stealBarScale*100+0.5)==sz) and Color3.fromRGB(60,60,80) or INP;sb.BorderSizePixel=0 
            sb.Text=tostring(sz);sb.TextColor3=(math.floor(stealBarScale*100+0.5)==sz) and WHITE or W;sb.Font=Enum.Font.SciFi;sb.TextSize=8;sb.ZIndex=5 
            Instance.new("UICorner",sb).CornerRadius=UDim.new(0,5) 
            local st=Instance.new("UIStroke",sb);st.Color=STROKE;st.Thickness=1 
            table.insert(refs,{btn=sb,sz=sz}) 
            sb.Activated:Connect(function() 
                stealBarScale=sz/100 
                if stealBarScaleObj then stealBarScaleObj.Scale=stealBarScale end 
                refresh(sz);saveConfig() 
            end) 
        end 
    end 
...
-- ============================================================ 
-- STEAL BAR – IBLIS HUB STYLE (avec fond image + resize) 
-- ============================================================ 
local StealBarGui = Instance.new("ScreenGui") 
StealBarGui.Name = "IblisStealBar" 
...
stealBarContainer.Size = UDim2.new(0, 320, 0, 100) 
stealBarContainer.Position = UDim2.new(0.5, -160, 0, 80) 
stealBarContainer.BackgroundTransparency = 1 
stealBarContainer.ZIndex = 10 

-- UIScale pour redimensionner dynamiquement
stealBarScaleObj = Instance.new("UIScale", stealBarContainer)
stealBarScaleObj.Scale = stealBarScale

local sbMain = Instance.new("Frame", stealBarContainer) 
sbMain.Size = UDim2.new(1, 0, 1, 0) 
sbMain.Position = UDim2.new(0, 0, 0, 0) 
sbMain.BackgroundColor3 = Color3.fromRGB(0, 0, 0) 
sbMain.BackgroundTransparency = 1
sbMain.BorderSizePixel = 0 
sbMain.ZIndex = 2 
Instance.new("UICorner", sbMain).CornerRadius = UDim.new(0, 8) 

-- ===== FOND IMAGE =====
local sbBgImage = Instance.new("ImageLabel", sbMain)
sbBgImage.Name = "BgImage"
sbBgImage.Size = UDim2.new(1, 0, 1, 0)
sbBgImage.Position = UDim2.new(0, 0, 0, 0)
sbBgImage.BackgroundTransparency = 1
sbBgImage.Image = "rbxassetid://133146589354241"
sbBgImage.ScaleType = Enum.ScaleType.Crop
sbBgImage.ZIndex = 2
Instance.new("UICorner", sbBgImage).CornerRadius = UDim.new(0, 8)

-- Overlay sombre pour lisibilité
local sbOverlay = Instance.new("Frame", sbMain)
sbOverlay.Size = UDim2.new(1, 0, 1, 0)
sbOverlay.Position = UDim2.new(0, 0, 0, 0)
sbOverlay.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
sbOverlay.BackgroundTransparency = 0.55
sbOverlay.BorderSizePixel = 0
sbOverlay.ZIndex = 3
Instance.new("UICorner", sbOverlay).CornerRadius = UDim.new(0, 8)

local sbBorder = Instance.new("UIStroke", sbMain) 
sbBorder.Color = Color3.fromRGB(255, 255, 255) 
sbBorder.Thickness = 1.5 
sbBorder.Transparency = 0.3 
sbBorder.ApplyStrokeMode = Enum.ApplyStrokeMode.Border 
sbBorder.ZIndex = 4

local progressBg = Instance.new("Frame", sbMain) 
progressBg.Name = "ProgressBg" 
progressBg.Size = UDim2.new(1, -16, 0, 18) 
...
local pbFrame = Instance.new("Frame", sbMain) 
pbFrame.Name = "PingFrame" 
pbFrame.Size = UDim2.new(1, -16, 0, 45) 
pbFrame.Position = UDim2.new(0, 8, 0, 30) 
pbFrame.BackgroundTransparency = 1 
pbFrame.ZIndex = 5

local pingBarLbl = Instance.new("TextLabel", pbFrame) 
pingBarLbl.Size = UDim2.new(0, 150, 0, 14) 
pingBarLbl.Position = UDim2.new(0, 0, 0, 0) 
...
progressRadLbl.Size = UDim2.new(0, 150, 0, 14) 
progressRadLbl.Position = UDim2.new(1, -150, 0, 0) 
progressRadLbl.BackgroundTransparency = 1 
progressRadLbl.Text = "Radius: 60" 
...
progressRadLbl.ZIndex = 6

local discordStealLbl = Instance.new("TextLabel", pbFrame) 
discordStealLbl.Size = UDim2.new(1, 0, 0, 14) 
discordStealLbl.Position = UDim2.new(0, 0, 0, 18) 
discordStealLbl.BackgroundTransparency = 1 
discordStealLbl.Text = "discord.gg/SrBSCVyKH" 
discordStealLbl.TextColor3 = Color3.fromRGB(180, 180, 180) 
discordStealLbl.Font = Enum.Font.GothamBold 
discordStealLbl.TextSize = 11 
discordStealLbl.TextXAlignment = Enum.TextXAlignment.Right 
discordStealLbl.ZIndex = 6
...
task.spawn(function() 
    ...
    while task.wait(0.5) do 
        ...
        pingBarLbl.Text = string.format("%d FPS | %dms", math.floor(fpsAvg + 0.5), ping) 
        pingBarLbl.TextColor3 = Color3.fromRGB(255, 255, 255) 
        progressRadLbl.Text = string.format("Radius: %s", tostring(Steal.StealRadius)) 
    end 
end) 
...
local function loadConfigKeys() 
    ...
    if cfg.stealBarScale and type(cfg.stealBarScale)=="number" then stealBarScale=math.clamp(cfg.stealBarScale,0.3,2.0) end 
...
local function loadConfigState() 
    ...
    task.spawn(function() 
        ...
        if cfg.antiDrop then startAntiDrop();if setAntiDropVisual then setAntiDropVisual(true) end end 
...
function resetFovAndCamera() 
    setFovEnabled(false) 
    if stretchRezEnabled then disableStretchRez();if setStretchRezVisual then setStretchRezVisual(true) end end 
    if workspace.CurrentCamera then workspace.CurrentCamera.FieldOfView=70 end 
end 

print("IBLIS HUB - discord.gg/SrBSCVyKH") 
